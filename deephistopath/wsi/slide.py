...
def get_filter_image_filename(
    ...
    img_filename = SLIDE_NAMES[slide_number-1] + "-" + padded_sl_num + "-" + padded_fi_num + \
...
def get_tile_summary_image_path(slide_number):
    ...
    img_path = os.path.join(TILE_SUMMARY_DIR,
...
def get_tile_summary_thumbnail_path(slide_number):
    ...
    img_path = os.path.join(
        ...
        get_tile_summary_image_filename(
            slide_number,
            thumbnail=True))
    return img_path
...
def get_tile_summary_image_filename(slide_number, thumbnail=False):
    ...
    large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(
        training_img_path)
    img_filename = SLIDE_NAMES[slide_number-1] + "-" + padded_sl_num + "-" + str(SCALE_FACTOR) + "x-" + str(large_w) + "x" + str(
...
def get_tile_data_filename(slide_number):
    ...
    large_w, large_h, small_w, small_h = parse_dimensions_from_image_filename(
        training_img_path)
    data_filename = SLIDE_NAMES[slide_number-1] + "-" + padded_sl_num + "-" + str(SCALE_FACTOR) + "x-" + str(large_w) + "x" + str(
...
def get_tile_data_path(slide_number):
    ...
    file_path = os.path.join(
        TILE_DATA_DIR,
        get_tile_data_filename(slide_number))
    return file_path
...
def show_scaled_slide_image(slide_filepath):
    """
    Convert a WSI training slide to a scaled-down PIL image.

    Args:
      slide_filepath: The wsi slide file path.
