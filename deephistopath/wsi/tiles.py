...
# To get around renderer issue on macOS going from Matplotlib image to
# NumPy image.
import matplotlib
...
# The size of the colored rectangular border around summary tiles.
TILE_BORDER_SIZE = 2
...
def generate_tile_summaries(
    ...
    summary_font = ImageFont.truetype(
        SUMMARY_TITLE_FONT_PATH,
        size=SUMMARY_TITLE_TEXT_SIZE)
    ...
    if DISPLAY_TILE_SUMMARY_LABELS:
        ...
        for t in tile_sum.tiles:
            ...
            draw.text(((t.c_s + 2), (t.r_s + 2 + z)), label,
                      SUMMARY_TILE_TEXT_COLOR, font=font)
            draw_orig.text(((t.c_s + 2), (t.r_s + 2 + z)), label,
                           SUMMARY_TILE_TEXT_COLOR, font=font)
...
def generate_top_tile_summaries(
    ...
    for t in top_tiles:
        ...
        tile_border(
            ...
            border_color)
        if border_all_tiles:
            tile_border(draw, t.r_s + z + tbs, t.r_e + z - tbs,
                        t.c_s + tbs, t.c_e - tbs, (0, 0, 0))
            tile_border(draw_orig, t.r_s + z + tbs, t.r_e + z -
                        tbs, t.c_s + tbs, t.c_e - tbs, (0, 0, 0))
    ...
    for t in tiles_to_label:
        ...
        draw.text(((t.c_s + h_offset), (t.r_s + v_offset + z)),
                  label, SUMMARY_TILE_TEXT_COLOR, font=font)
        draw_orig.text(
            ((t.c_s + h_offset),
             (t.r_s + v_offset + z)),
            label,
...
def score_tiles(
    ...
    num_row_tiles, num_col_tiles = get_num_tiles(
        h, w, row_tile_size, col_tile_size)
    ...
    for t in tile_indices:
        ...
        # pixel adjustment in case tile dimension too large (for example, 1025
        # instead of 1024)
...
def singleprocess_filtered_images_to_tiles(
    ...
    if html:
        generate_tiled_html_result(
            image_num_list, tile_summaries_dict, save_data)
...
def image_row(slide_num, tile_summary, data_link):
    ...
    # sort top tiles by rows and columns to make them easier to locate on HTML
    # page
    ...
    for row in range(num_rows):
        ...
        for col in range(num_cols):
            ...
            if tile_num <= num_tiles:
                ...
                label = "R%03d C%03d %0.4f (#%02d)" % (
                    t.r, t.c, t.score, t.tile_num)
...
def np_hsv_hue_histogram(h):
    ...
    plt.title(
        "HSV Hue Histogram, mean=%3.1f, std=%3.1f" %
        (np.mean(h), np.std(h)))
    ...
    np_hist = np.fromstring(
        canvas.get_renderer().tostring_rgb(),
...
def np_hsv_saturation_histogram(s):
    ...
    """
    title = "HSV Saturation Histogram, mean=%.2f, std=%.2f" % (
        np.mean(s), np.std(s))
...
def np_hsv_value_histogram(v):
    ...
    Returns:
      Matplotlib histogram of saturation values converted to a NumPy array image.
    """
    title = "HSV Value Histogram, mean=%.2f, std=%.2f" % (
        np.mean(v), np.std(v))
...
def np_rgb_channel_histogram(rgb, ch_num, ch_name):
    ...
    ch = rgb[:, :, ch_num]
    ch = ch.flatten()
    title = "RGB %s Histogram, mean=%.2f, std=%.2f" % (
        ch_name, np.mean(ch), np.std(ch))
...
def display_image_with_rgb_and_hsv_histograms(
        np_rgb, text=None, scale_up=False):
    """
    Display a tile with its corresponding RGB and HSV histograms.
